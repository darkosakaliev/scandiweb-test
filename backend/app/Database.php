...
class Database
{
    ...
    public function count(string $table, array $where)
    {
        $clause = array_reduce(
            array_keys($where),
            fn($total, $item) => $total . "$item = ? AND ",
            ""
        );

        $stmt = static::$connection->prepare(
            "SELECT COUNT(*) FROM $table WHERE " . rtrim($clause, " AND ")
        );
    ...
    public function delete(string $table, array $where)
    {
        ...
        $stmt = static::$connection->prepare(
            "DELETE FROM $table WHERE " . rtrim($clause, " AND ")
        );
    ...
    public function deleteMany(string $table, array $ids)
    {
        ...
        $stmt = static::$connection->prepare(
            "DELETE FROM $table WHERE id IN (" . rtrim($clause, ", ") . ")"
        );
